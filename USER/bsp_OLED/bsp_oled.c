...
/*------------------------------------------------------------*/
	...
	OLED_CMD(0xff);
	OLED_CMD(0xa1);//--set segment re-map 0 to 127	ZY a8
...
/*-------------------Flash------------------------------*/
void OLED_flash_8_Fast(uint8_t x, int y, uint8_t n)
{
	Draw_bmp(x,y,&Char_8[n][0],8,8);
}

uint8_t OLED_flash_Show_Char_8(uint8_t x,int y,uint8_t C)
{
	if(C < 32 || C > 126)
	{
		return 0;
	}
	OLED_flash_8_Fast(x,y,C - 32);
	
	return 1;
}

void OLED_flash_String_8(uint8_t x,int y,const char *C)
{
	while(*C != '\0')
	{
		if(x > 63)
...
void Draw_bmp(uint8_t x, int y, const uint8_t *bmp, uint8_t w, uint8_t h)
{
	uint8_t i;
	uint8_t j;
