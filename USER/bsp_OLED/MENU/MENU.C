...
bool Menu_Option(uint8_t num, const char* name, const uint8_t* icon, menu_f actionFunc)
{
	...
	switch(Menu_operation.op)
	{
		...
		case OPERATION_DRAWNAME_STR:Draw_Str(Menu_operation.data,0,name);break;		//绘制项目字符
...
{
	Set_Menu_Option(Main_Menu.optionCount - 1, "Exit", &MAIN_MUNE[4][0], Exit);
}

//返回上级菜单，上级只有打开动画过度
void Exit(void)
{
	Main_Menu.prevMenu != NULL ? Main_Menu.prevMenu() : Main_Menu.prevMenu()/*mMainOpen()*/;
	//mMainOpen();
}
...
{
	Draw_bmp(15,44,UP,40,5);
	Draw_bmp(44,44,DOWN,40,5);
}
